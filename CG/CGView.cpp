...
BEGIN_MESSAGE_MAP(CGView, CView)
...
BOOL CGView::OnPreparePrinting(CPrintInfo* pInfo)
{
	// default preparation
	return DoPreparePrinting(pInfo);
}

void CGView::OnBeginPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
	// TODO: add extra initialization before printing
}

void CGView::OnEndPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
...
CTerrain3DTestDoc* CGView::GetDocument() // non-debug version is inline
{
...
int CGView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CGView::OnSize(UINT nType, int cx, int cy)
{
...
void CGView::OnTimer(UINT nIDEvent)
{
...
//	                  设置逻辑调色板
//////////////////////////////////////////////////////////////////////
void CGView::SetLogicalPalette(void)
{
	struct
	{
		WORD Version;
...
//						初始化openGL场景
//////////////////////////////////////////////////////////
BOOL CGView::InitializeOpenGL(CDC* pDC)
{
	m_pDC = pDC;
	SetupPixelFormat();
	//生成绘制描述表
...
BOOL CGView::RenderScene()
{
	...
	glLoadIdentity();
	glTranslatef(0, 0, -10);    //每次向Z轴平移，模拟前进的效果
	glRotatef(sceneroty, 0, 1, 0);  //旋转
...
bool CGView::InitGL(GLvoid)
{
	...
	float fog_r = 211.f / 255.f;                       //设置烟雾颜色
	...
	RestoreMyDefaultSettings();
	glHint(GL_PERSPECTIVE_CORRECTION_HINT, GL_NICEST);	// 指定颜色和纹理坐标的差值质量  Really Nice Perspective Calculations
														// 设置光照效果
...
GLfloat CGView::Hypot(GLfloat A, GLfloat B)
{
	return sqrt(A*A + B*B);
}

GLfloat CGView::ABS(GLfloat A)
{
	if (A < 0)
		A = -A;
	return A;
}
void CGView::RestoreMyDefaultSettings()
{
...
bool CGView::InitTerrain(GLvoid)
{
	int i, i2;
...
bool CGView::DrawWireframe(GLvoid)
{
	int i;
	int i2;
	int t, t2;
...
bool CGView::DrawTexture(GLvoid)
{
	DrawTerrain();		// 绘制地形
	DrawSky();			// 绘制天空
	if (water)
...
bool CGView::DrawTerrain(GLvoid)
{
	int i;
	int i2;
	int t, t2;
